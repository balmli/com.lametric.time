...
module.exports = class LametricClient {
    ...
    async appAction(widget, params) {
        const body = {
            id: widget.id,
            params: params
        };
        const result = await this.post(`/api/v2/device/apps/${widget.appId}/widgets/${widget.id}/actions`, body);
        if (result.response.statusCode !== 201) {
            const statusCode = result.response.statusCode;
            const statusMessage = result.response.statusMessage;
            throw new Error(this.i18n.__('errors.app_action', { widgetName, statusCode, statusMessage }));
        }
        const data = result.data;
        this.log('App action', widget, body, data);
    }
