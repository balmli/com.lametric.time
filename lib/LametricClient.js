...
module.exports = class LametricClient {
    ...
    async apiVersion() {
        ...
        const data = result.data;
        this.log('Api version', JSON.stringify(data));
        return data;
    }
    ...
    async deviceState() {
        ...
        const data = result.data;
        this.log('Device state', JSON.stringify(data));
        return data;
    }

    async changeDeviceMode(mode) {
        const result = await this.put('/api/v2/device', { mode });
        if (result.response.statusCode === 405) {
            throw new Error(this.i18n.__('errors.change_device_mode_api_version'));
        } else if (result.response.statusCode !== 200) {
            const statusMessage = result.data.errors && result.data.errors.length > 0 ? result.data.errors[0].message : result.response.statusMessage;
            const statusCode = result.response.statusCode;
            throw new Error(this.i18n.__('errors.change_device_mode_failed', { statusCode, statusMessage }));
        }
    }
    ...
    async sendNotification(args) {
        this.log('sendNotification', args.priority, args.icontype, args.text, args.iconCode);
    ...
    _getNotificationBody(args) {
        return {
            ...
            model: {
                frames: [
                    {
                        icon: args.icon ? args.icon.code :
                            args.iconCode && !args.iconCode.includes('data:') ?
                                args.iconCode.replace(/\s+/g, '').toLowerCase() :
                                args.iconCode ? args.iconCode : undefined,
                        text: args.text
                    }
                ],
                sound: args.sound ? {
